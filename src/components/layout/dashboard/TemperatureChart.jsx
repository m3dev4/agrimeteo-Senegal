...
const TemperatureChart = ({ weatherData }) => {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      if (hour >= 6 && hour <= 12)        variation = (hour - 6) * 0.3;
      else if (hour > 12 && hour <= 18)   variation = 1.8 - (hour - 12) * 0.2;
      else if (hour > 18 && hour <= 24)   variation = 0.6 - (hour - 18) * 0.1;
      else                                variation = -1.2 + hour * 0.05;
  ...
  const labelMap = { max: "Maximum", avg: "Moyenne", min: "Minimum" };
    ...
    <div className="w-full text-slate-200">
      ...
      <div className="h-44 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 6, right: 6, left: -25, bottom: 0 }}>
            <defs>
            ...
            <YAxis domain={[min, max]} tick={{ fill: "#94a3b8", fontSize: 10 }} axisLine={false} tickLine={false} width={35} tickFormatter={(v) => `${v}°`} />
            <Tooltip
              contentStyle={{ background: "rgba(15,23,42,0.95)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: "#f8fafc", fontSize: 11 }}
              labelStyle={{ color: "#cbd5e1" }}
              formatter={(v, name) => [`${v}°C`, labelMap[name] ?? name]}
            />
            <Area type="monotone" dataKey="max" stroke="#ff9800" strokeWidth={1.5} fill="url(#maxGrad)" animationDuration={500} />
            <Area type="monotone" dataKey="avg" stroke="#4caf50" strokeWidth={2}   fill="url(#avgGrad)" animationDuration={500} />
            <Area type="monotone" dataKey="min" stroke="#42a5f5" strokeWidth={1.5} fill="url(#minGrad)" animationDuration={500} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
